...
def show_start_screen():
    ...
    # --- Webcam preview and Start button columns ---
    col_webcam, col_button = st.columns([1, 1])
    ...
    with col_webcam:
        st.markdown("<h4 style='text-align:center;'>📷 웹캠 미리보기</h4>", unsafe_allow_html=True)
        webcam_preview = st.empty()
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            webcam_preview.warning("⚠️ 웹캠을 사용할 수 없습니다.")
        else:
            while True:
                ret, frame = cap.read()
                if not ret:
                    webcam_preview.warning("⚠️ 웹캠 프레임을 가져오지 못했습니다.")
                    break
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                webcam_preview.image(frame, channels="RGB", use_container_width=True)
                time.sleep(1/30)
...
def reset_stage_state():
    for key in ["pose_matched", "replay_video", "final_image", "success_triggered", "video_header_shown", "webcam_header_shown"]:
...
def run_stage(video):
    ...
    with col1:
        if current > 0:
            if st.button("⏪ 이전으로"):
                if "video_container" in st.session_state:
                    st.session_state.video_container.empty()
                    del st.session_state.video_container
                if "webcam_container" in st.session_state:
                    st.session_state.webcam_container.empty()
                    del st.session_state.webcam_container
                reset_stage_state()
    ...
    with left:
        ...
        if st.session_state.get("replay_video", False):
            ...
            video_container.image(thumbnail_path, use_container_width=True)

    with right:
        if not st.session_state.get("webcam_header_shown", False):
            st.markdown("<h4 style='text-align:center;'>📷 실시간 웹캠</h4>", unsafe_allow_html=True)
            st.session_state.webcam_header_shown = True
        if "webcam_container" not in st.session_state:
            st.session_state.webcam_container = st.empty()
        stframe = st.session_state.webcam_container
        ...
        if not st.session_state.show_countdown: 
            ...
            while True:
                ...
                # 2초(=60프레임 @30fps) 후에는 무조건 성공 처리
                if frame_count >= 300:
                    st.session_state.pose_matched = True
                    st.session_state.replay_video = True
                    if not st.session_state.get("success_triggered", False):
                        #status_placeholder.success("🎯 너무 잘했어요!")
                        st.session_state.success_triggered = True
                        #st.balloons()
                        time.sleep(1)
                        st.rerun()
